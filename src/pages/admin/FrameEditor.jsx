...
import { ArrowLeft, Save, Plus, Trash2, Move, Maximize2, Sparkles } from 'lucide-react';
    ...
    return (
        <div className="min-h-[calc(100vh-6rem)] lg:h-[calc(100vh-5rem)] font-nunito bg-slate-50 text-slate-800 flex flex-col relative w-full max-w-6xl mx-auto">
            ...
            <div className="shrink-0 w-full flex items-center justify-between mb-4 z-10 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                <button onClick={() => navigate('/admin/frames')} className="flex items-center gap-2 text-slate-500 hover:text-slate-800 font-semibold px-2 py-1 transition">
                    <ArrowLeft size={18} /> Back
                </button>
                <div className="flex items-center gap-3">
                    <Maximize2 className="text-blue-500" size={24} />
                    <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 tracking-tight">{editingFrame ? 'Edit Frame Layout' : 'New Frame Layout'}</h1>
                </div>
                <button
                    onClick={handleSave}
                    disabled={uploading}
                    className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-blue-700 transition flex items-center gap-2 disabled:opacity-50 shadow-sm"
                >
                ...
                <div className="lg:col-span-4 bg-white p-5 lg:p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-full overflow-hidden">
                    ...
                    <div className="shrink-0 flex gap-2 mb-4 border-b border-slate-100 pb-4">
                        <button
                            ...
                            className={`flex-1 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'metadata' ? 'bg-blue-50 text-blue-600 shadow-sm border border-blue-100' : 'bg-white text-slate-500 hover:bg-slate-50 border border-slate-100'}`}
                        >
                            <Sparkles size={18} className={activeTab === 'metadata' ? 'text-blue-500' : 'text-slate-400'} /> Metadata
                        </button>
                        <button
                            onClick={() => setActiveTab('slots')}
                            className={`flex-1 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'slots' ? 'bg-emerald-50 text-emerald-600 shadow-sm border border-emerald-100' : 'bg-white text-slate-500 hover:bg-slate-50 border border-slate-100'}`}
                        >
                            <Move size={18} className={activeTab === 'slots' ? 'text-emerald-500' : 'text-slate-400'} /> Slots
                        </button>
                    </div>
